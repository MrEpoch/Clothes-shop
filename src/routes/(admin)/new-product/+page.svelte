	...
	<div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
		...
		<form method="POST" enctype="multipart/form-data">
			<div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                ...
                <div class="h-80 relative sm:col-span-2">
                    {#if image}
                        <img class="rounded-lg w-full h-full object-cover" src={URL.createObjectURL(image[0])} alt="" />
                    {:else}
                        <div class="rounded-lg w-full bg-gray-50 dark:bg-gray-700 h-full flex items-center flex-col gap-5 justify-center ">
                            <FileIcon class="w-10 h-10" />
                            <p>Upload your file here</p>
                            <p>PNG, JPG, JPEG, WEBP allowed.</p>
                        </div>
                    {/if}
                    <input type="file" required bind:files={image} 
                    class="absolute w-full h-full z-10 top-0 left-0 opacity-0 cursor-pointer" name="image" />
                </div>
			</div>
			<button
				type="submit"
